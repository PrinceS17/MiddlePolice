...
void MyApp::SetDataRate(DataRate rate)
{
  m_dataRate = rate;
}

void MyApp::SetTagValue(uint32_t value)
{
  m_tagValue = value;
}
...
PktArrival(Ptr<const Packet> p)
{
  ...
  if (Simulator::Now().GetSeconds() > detectPeriod)
  {
    ...
    for (uint32_t j = 0; j < nLeaf + nAttacker; ++j)
    {
      ...
      if (j > 0)
      {
        attacker_windows[j % 3] += congWin[j]; /// why j % 3? update attacker window with the largest j 's window data?
